...
//
// FIFA's real Round-of-32 draw uses a 495-scenario lookup table to place
// each group's 3rd-placed team into a wildcard slot, which can't be
// reproduced offline. Instead we apply this fixed, deterministic pairing
// to both the predicted bracket (from a player's GroupPrediction) and the
// reference bracket (from the admin-entered GroupResult), so every player
// is compared against the same structure.
